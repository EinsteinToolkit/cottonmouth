...
void z4c_apply_NewRadX_Theta(CCTK_ARGUMENTS) {
    DECLARE_CCTK_ARGUMENTSX_z4c_apply_NewRadX_Theta;
    DECLARE_CCTK_PARAMETERS;
    using vreal = CCTK_REAL;
    constexpr std::size_t vsize = 0;
    #ifdef __CUDACC__
    const nvtxRangeId_t range = nvtxRangeStartA("z4c_apply_NewRadX_Theta");
    #endif
    NewRadX_Apply(cctkGH, Theta, Theta_rhs, 0, 1, radpower_Theta);
    #ifdef __CUDACC__
    nvtxRangeEnd(range);
    #endif
}