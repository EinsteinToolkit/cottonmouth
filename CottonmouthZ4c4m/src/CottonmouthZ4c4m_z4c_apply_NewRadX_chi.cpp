...
void z4c_apply_NewRadX_chi(CCTK_ARGUMENTS) {
    DECLARE_CCTK_ARGUMENTSX_z4c_apply_NewRadX_chi;
    DECLARE_CCTK_PARAMETERS;
    using vreal = CCTK_REAL;
    constexpr std::size_t vsize = 0;
    #ifdef __CUDACC__
    const nvtxRangeId_t range = nvtxRangeStartA("z4c_apply_NewRadX_chi");
    #endif
    NewRadX_Apply(cctkGH, chi, chi_rhs, 1, 1, radpower_chi);
    #ifdef __CUDACC__
    nvtxRangeEnd(range);
    #endif
}