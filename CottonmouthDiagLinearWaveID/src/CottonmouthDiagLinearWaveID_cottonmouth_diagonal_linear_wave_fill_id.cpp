...
void cottonmouth_diagonal_linear_wave_fill_id(CCTK_ARGUMENTS) {
    ...
    grid.loop_all_device<VVV_centered[0], VVV_centered[1], VVV_centered[2], vsize>(grid.nghostzones, [=] CCTK_DEVICE(const PointDesc& p) CCTK_ATTRIBUTE_ALWAYS_INLINE {    
        ...
        vreal x2 = ((1.0 / 2.0) * x0 * x1 * ((6.28318530717959 * x) + (-6.28318530717959 * y) + (-6.28318530717959 * t * x0)));
        x0 = (3.14159265358979 * amplitude * x1 * cos(x2));
        x1 = (amplitude * sin(x2));
        x2 = (19.7392088021787 * x1 * pown<vreal>(wavelength, -2));
        store(gzz, stencil_idx_0_0_0_VVV, (1 + (-(x1))));
        store(gyy, stencil_idx_0_0_0_VVV, (1 + x1));
        store(dtkzz, stencil_idx_0_0_0_VVV, (-(x2)));
        store(dtkyy, stencil_idx_0_0_0_VVV, x2);
        store(kzz, stencil_idx_0_0_0_VVV, (-(x0)));
        store(kyy, stencil_idx_0_0_0_VVV, x0);
