...
void bssnok_apply_NewRadX_evo_shift(CCTK_ARGUMENTS) {
    DECLARE_CCTK_ARGUMENTSX_bssnok_apply_NewRadX_evo_shift;
    DECLARE_CCTK_PARAMETERS;
    using vreal = CCTK_REAL;
    constexpr std::size_t vsize = 0;
    #ifdef __CUDACC__
    const nvtxRangeId_t range = nvtxRangeStartA("bssnok_apply_NewRadX_evo_shift");
    #endif
    NewRadX_Apply(cctkGH, evo_shiftU0, evo_shift_rhsU0, 0, 1, radpower_evo_shift);
    NewRadX_Apply(cctkGH, evo_shiftU1, evo_shift_rhsU1, 0, 1, radpower_evo_shift);
    NewRadX_Apply(cctkGH, evo_shiftU2, evo_shift_rhsU2, 0, 1, radpower_evo_shift);
    #ifdef __CUDACC__
    nvtxRangeEnd(range);
    #endif
}