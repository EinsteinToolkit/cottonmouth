...
void bssnok_apply_NewRadX_trK(CCTK_ARGUMENTS) {
    DECLARE_CCTK_ARGUMENTSX_bssnok_apply_NewRadX_trK;
    DECLARE_CCTK_PARAMETERS;
    using vreal = CCTK_REAL;
    constexpr std::size_t vsize = 0;
    #ifdef __CUDACC__
    const nvtxRangeId_t range = nvtxRangeStartA("bssnok_apply_NewRadX_trK");
    #endif
    NewRadX_Apply(cctkGH, trK, trK_rhs, 0, 1, radpower_trK);
    #ifdef __CUDACC__
    nvtxRangeEnd(range);
    #endif
}