...
void bssnok_apply_NewRadX_shift_B(CCTK_ARGUMENTS) {
    DECLARE_CCTK_ARGUMENTSX_bssnok_apply_NewRadX_shift_B;
    DECLARE_CCTK_PARAMETERS;
    using vreal = CCTK_REAL;
    constexpr std::size_t vsize = 0;
    #ifdef __CUDACC__
    const nvtxRangeId_t range = nvtxRangeStartA("bssnok_apply_NewRadX_shift_B");
    #endif
    NewRadX_Apply(cctkGH, shift_BU0, shift_B_rhsU0, 0, 1, radpower_shiftB);
    NewRadX_Apply(cctkGH, shift_BU1, shift_B_rhsU1, 0, 1, radpower_shiftB);
    NewRadX_Apply(cctkGH, shift_BU2, shift_B_rhsU2, 0, 1, radpower_shiftB);
    #ifdef __CUDACC__
    nvtxRangeEnd(range);
    #endif
}