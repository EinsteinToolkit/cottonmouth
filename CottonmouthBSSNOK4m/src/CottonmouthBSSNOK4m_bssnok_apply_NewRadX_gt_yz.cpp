...
void bssnok_apply_NewRadX_gt_yz(CCTK_ARGUMENTS) {
    DECLARE_CCTK_ARGUMENTSX_bssnok_apply_NewRadX_gt_yz;
    DECLARE_CCTK_PARAMETERS;
    using vreal = CCTK_REAL;
    constexpr std::size_t vsize = 0;
    #ifdef __CUDACC__
    const nvtxRangeId_t range = nvtxRangeStartA("bssnok_apply_NewRadX_gt_yz");
    #endif
    NewRadX_Apply(cctkGH, gtDD12, gt_rhsDD12, 0, 1, radpower_gt);
    #ifdef __CUDACC__
    nvtxRangeEnd(range);
    #endif
}