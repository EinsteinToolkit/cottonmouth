...
void bssnok_apply_NewRadX_evo_lapse(CCTK_ARGUMENTS) {
    DECLARE_CCTK_ARGUMENTSX_bssnok_apply_NewRadX_evo_lapse;
    DECLARE_CCTK_PARAMETERS;
    using vreal = CCTK_REAL;
    constexpr std::size_t vsize = 0;
    #ifdef __CUDACC__
    const nvtxRangeId_t range = nvtxRangeStartA("bssnok_apply_NewRadX_evo_lapse");
    #endif
    NewRadX_Apply(cctkGH, evo_lapse, evo_lapse_rhs, 1, 1, radpower_evo_lapse);
    #ifdef __CUDACC__
    nvtxRangeEnd(range);
    #endif
}