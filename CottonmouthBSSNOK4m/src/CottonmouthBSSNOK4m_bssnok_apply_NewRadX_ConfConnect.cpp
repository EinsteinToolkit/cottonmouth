...
void bssnok_apply_NewRadX_ConfConnect(CCTK_ARGUMENTS) {
    DECLARE_CCTK_ARGUMENTSX_bssnok_apply_NewRadX_ConfConnect;
    DECLARE_CCTK_PARAMETERS;
    using vreal = CCTK_REAL;
    constexpr std::size_t vsize = 0;
    #ifdef __CUDACC__
    const nvtxRangeId_t range = nvtxRangeStartA("bssnok_apply_NewRadX_ConfConnect");
    #endif
    NewRadX_Apply(cctkGH, ConfConnectU0, ConfConnect_rhsU0, 0, 1, radpower_ConfConnect);
    NewRadX_Apply(cctkGH, ConfConnectU1, ConfConnect_rhsU1, 0, 1, radpower_ConfConnect);
    NewRadX_Apply(cctkGH, ConfConnectU2, ConfConnect_rhsU2, 0, 1, radpower_ConfConnect);
    #ifdef __CUDACC__
    nvtxRangeEnd(range);
    #endif
}