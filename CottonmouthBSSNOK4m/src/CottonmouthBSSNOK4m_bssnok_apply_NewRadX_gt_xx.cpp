...
void bssnok_apply_NewRadX_gt_xx(CCTK_ARGUMENTS) {
    DECLARE_CCTK_ARGUMENTSX_bssnok_apply_NewRadX_gt_xx;
    DECLARE_CCTK_PARAMETERS;
    using vreal = CCTK_REAL;
    constexpr std::size_t vsize = 0;
    #ifdef __CUDACC__
    const nvtxRangeId_t range = nvtxRangeStartA("bssnok_apply_NewRadX_gt_xx");
    #endif
    NewRadX_Apply(cctkGH, gtDD00, gt_rhsDD00, 1, 1, radpower_gt);
    #ifdef __CUDACC__
    nvtxRangeEnd(range);
    #endif
}