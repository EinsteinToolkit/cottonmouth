...
void cottonmouth_linear_wave_fill_id(CCTK_ARGUMENTS) {
    ...
    grid.loop_all_device<VVV_centered[0], VVV_centered[1], VVV_centered[2], vsize>(grid.nghostzones, [=] CCTK_DEVICE(const PointDesc& p) CCTK_ATTRIBUTE_ALWAYS_INLINE {    
        ...
        vreal x3 = (amplitude * sin(x1));
        vreal x4 = (19.7392088021787 * x3 * pown<vreal>(wavelength, -2));
        vreal x2 = (3.14159265358979 * amplitude * x0 * cos(x1));
        store(gzz, stencil_idx_0_0_0_VVV, (1 + (-(x3))));
        store(gyy, stencil_idx_0_0_0_VVV, (1 + x3));
        store(kzz, stencil_idx_0_0_0_VVV, (-(x2)));
        store(dtkzz, stencil_idx_0_0_0_VVV, (-(x4)));
        ...
        store(dtkyy, stencil_idx_0_0_0_VVV, x4);
