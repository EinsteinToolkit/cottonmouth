...
void cottonmouth_gauge_wave_fill_id(CCTK_ARGUMENTS) {
    DECLARE_CCTK_ARGUMENTSX_cottonmouth_gauge_wave_fill_id;
    DECLARE_CCTK_PARAMETERS;
    using vreal = CCTK_REAL;
    constexpr std::size_t vsize = 0;
    #ifdef __CUDACC__
    const nvtxRangeId_t range = nvtxRangeStartA("cottonmouth_gauge_wave_fill_id");
    #endif
    ...
    // cottonmouth_gauge_wave_fill_id loop 0
    grid.loop_all_device<VVV_centered[0], VVV_centered[1], VVV_centered[2]>(grid.nghostzones, [=] CCTK_DEVICE(const PointDesc& p) CCTK_ATTRIBUTE_ALWAYS_INLINE {    
        ...
        vreal x1 = pown<vreal>(wavelength, -1);
        vreal x2 = (x1 * ((6.28318530717959 * x) + (-6.28318530717959 * t)));
        vreal x3 = sin(x2);
        vreal x4 = (1 + (-1 * amplitude * x3));
        vreal x0 = pown<vreal>(wavelength, -2);
        vreal x5 = sqrt(x4);
        vreal x6 = pown<vreal>(x5, -1);
        vreal x7 = cos(x2);
        x2 = ((9.86960440108936 * x0 * pow2(amplitude) * pow(static_cast<vreal>(x4), (-3.0 / 2.0)) * pow2(x7)) + (-19.7392088021787 * amplitude * x0 * x3 * x6));
        x0 = (3.14159265358979 * amplitude * x1 * x6 * x7);
        store(gxx, stencil_idx_0_0_0_VVV, x4);
        store(alp, stencil_idx_0_0_0_VVV, x5);
        store(dt2alp, stencil_idx_0_0_0_VVV, (-(x2)));
        store(dtkxx, stencil_idx_0_0_0_VVV, x2);
        store(kxx, stencil_idx_0_0_0_VVV, (-(x0)));
        store(dtalp, stencil_idx_0_0_0_VVV, x0);
